...
bool triTest(float x, float y, float x0, float y0, float x1, float y1, float x2, float y2){
        bool b0 = lineEquation(x,y,x0,y0,x1,y1) >= 0;
        bool b1 = lineEquation(x,y,x1,y1,x2,y2) >= 0;
        bool b2 = lineEquation(x,y,x2,y2,x0,y0) >= 0;
...
void DrawRend::rasterize_triangle( float x0, float y0,
    ...
    int floorx = floor(min(x0, min(x1, x2)));
    int topx = ceil(max(x0, max(x1, x2)));
    int floory = floor(min(y0, min(y1, y2)));
    int topy = ceil(max(y0, max(y1, y2)));
    int sample_per_pixel_one_axis = sqrt(sample_rate);
    float subLength = 1.0 / sample_per_pixel_one_axis;
    
    float x = 0;
    float y = 0;
    
    for (int i = floory; i < topy; ++i){ //interate throuth y
        for (int k = floorx; k < topx; ++k){ //interate throuth x
            for (int a = 0; a < sample_per_pixel_one_axis; ++a){ //sun row
                for (int b = 0; b < sample_per_pixel_one_axis; ++b){ //sub col
        
                    x = k + a * subLength + subLength /2.0; //1/2 sub pixel
                    y = i + b * subLength + subLength /2.0;
                    if (triTest(x, y, x0, y0, x1, y1, x2, y2)){
                        if (tri != NULL) {
                            SampleParams sp = SampleParams();
                            sp.lsm = lsm;
                            sp.psm = psm;
                            
                            float alp = lineEquation(x, y, x1, y1, x2, y2) /
                                        lineEquation(x0, y0, x1, y1, x2, y2);
                            
                            float bet = lineEquation(x, y, x2, y2, x0, y0) /
                                        lineEquation(x1, y1, x2, y2, x0, y0);
                            float cet = 1 - alp - bet;
                            Vector3D coor = Vector3D(alp, bet, cet);
                            
                            float alpDx = lineEquation(x+1, y, x1, y1, x2, y2) /
                            lineEquation(x0, y0, x1, y1, x2, y2);
                            float betDx = lineEquation(x+1, y, x2, y2, x0, y0) /
                            lineEquation(x1, y1, x2, y2, x0, y0);
                            float cetDx = 1 - alpDx - betDx;
                            Vector3D byCoorDx = Vector3D(alpDx, betDx, cetDx);
                            
                            float alpDy = lineEquation(x, y+1, x1, y1, x2, y2) /
                            lineEquation(x0, y0, x1, y1, x2, y2);
                            float betDy = lineEquation(x, y+1, x2, y2, x0, y0) /
                            lineEquation(x1, y1, x2, y2, x0, y0);
                            float cetDy = 1 - alpDy - betDy;
                            Vector3D byCoorDy = Vector3D(alpDy, betDy, cetDy);
                            
                            samplebuffer[i][k].fill_color(a,b,tri->color(coor, byCoorDx, byCoorDy, sp));
                        }
                        else{
                            samplebuffer[i][k].fill_color(a, b, color);
                        }
                    }
                }
            }
